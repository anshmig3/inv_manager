...
const selectCls = "text-sm border border-slate-600 rounded-lg px-2 py-1.5 bg-slate-700 text-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-500";
...
export default function SKUFilters({ filters, categories, onChange }: Props) {
  ...
  return (
    <div className="bg-slate-800 rounded-2xl border border-slate-700 p-4 mb-4 flex flex-wrap gap-3 items-center">
      <SlidersHorizontal size={16} className="text-cyan-400 shrink-0" />
      ...
      <div className="relative flex-1 min-w-48">
        ...
        <input
          ...
          onChange={set('search')}
          className="w-full pl-8 pr-3 py-1.5 text-sm border border-slate-600 rounded-lg bg-slate-700 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500"
        />
      </div>
      ...
      <select value={filters.promo} onChange={set('promo')} className={selectCls}>
